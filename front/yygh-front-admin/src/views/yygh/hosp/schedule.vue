...
import schApi from '@/api/schedule'
export default {
    data() {
        return {
            data: [],
    ...
    methods:{
        fetchData() {
            hospApi.getDeptByHoscode(this.hoscode)
                .then(response => {
                    this.data = response.data.departmentList
                    // 默认选中第一个
        ...
        getScheduleRule() {
            schApi.getSchedulePage(this.pageNum, this.pageSize, this.hoscode, this.depcode).then(response => {
        ...
        getDetailSchedule(){
            schApi.detail(this.hoscode,this.depcode,this.workDate).then(resp=>{
