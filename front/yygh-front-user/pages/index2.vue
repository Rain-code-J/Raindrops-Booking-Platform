...
export default {
  ...
  methods:{
    ...
    init() {
      ...
      dictApi.findByParentId('10000')
        .then(response => {
          ...
          for(var i=0;i<response.data.dictList.length;i++) {
            this.hostypeList.push(response.data.dictList[i])
          }
        })

      //查询地区数据
      dictApi.findByParentId('110100')
        .then(response => {
          this.districtList = []
    ...
    //查询医院列表
    getList() {
      hospApi.getPageList(this.searchObj)
        .then(response => {
          for(let i in response.data.hospitalList) {
            this.list.push(response.data.hospitalList[i])
          }
    ...
    querySearchAsync(queryString, cb) {
      ...
      hospApi.getByHosname(queryString).then(response => {
        for (let i = 0, len = response.data.hospitalQueryList.length; i <len; i++) {
          response.data.hospitalQueryList[i].value = response.data.hospitalQueryList[i].hosname
        }
        cb(response.data.hospitalQueryList)
      })
    },
